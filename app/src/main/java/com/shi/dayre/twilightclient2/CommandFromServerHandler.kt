...
//            Log.i("WebClient.Proceed", "Proceed JSON name " + jsonObject.getString("name"))
        ...
        if (fromServer.startsWith("ZONE(")) {
            ...
            if (service.userState.zoneText != fromServer.getTextBetween()[0] && fromServer.getTextBetween()[0]!="noText") {
                //Change zone
            ...
            if (service.userState.vampireCall != fromServer.getTextBetween()[2]) {
                //Vampire Call
                val n = fromServer.getTextBetween()[2]
                service.userState.vampireCall = n
                if (n.equals("0"))
                    sendNotification(context, context.getString(R.string.dismissVampireCall), "", NOTIFICATION_CODE_VAMPIRE, 0)
                else
                    sendNotification(context, context.getString(R.string.newVampireCall), "", NOTIFICATION_CODE_VAMPIRE, 1)
            }
            //Vampire send
            val n = fromServer.getTextBetween()[3]
            service.userState.vampireSend = n
            //And refresh
            sendNeedToRefresh("all")
            service.waitinForServerResponse = false
